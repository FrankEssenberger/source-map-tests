...
const remotePathToSources =
  "raw.githubusercontent.com/SAP/cloud-sdk-js/main/packages/util/src/";
const protocol = "https://";
const pathToDist = resolve(
  __dirname,
  "../",
  "node_modules",
  "@sap-cloud-sdk",
  ...
  await Promise.all(
    files.map(async (file) => {
      try {
        ...
        const newSourceRoot = useSourceRoot
          ? protocol + join(remotePathToSources, fileFolder)
          : "";
        const sourceFileName = fileName!.replace(
          new RegExp(".js.map|.d.ts.map"),
          ".ts"
        );
        const newSources = useSourceRoot
          ? sourceFileName
          : protocol + join(remotePathToSources, fileFolder, fileName!);
        const newSourceMap = {
          ...fileContent,
        ...
        await writeFile(filePath, JSON.stringify(newSourceMap), {
          encoding: "utf-8",
        });
      } catch (e: any) {
